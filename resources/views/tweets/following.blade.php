...
@section('title', 'フォロー中のツイート')

@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="fs-4 mb-0"><i class="fas fa-users"></i> フォロー中のツイート</h1>
    ...
    <div id="tweets-container" class="tweets-container">
        @foreach ($tweets as $tweet)
            <x-tweet-card :tweet="$tweet" :showControls="Auth::id() === $tweet->user_id" />
        @endforeach
        
        @if ($tweets->isEmpty())
            <div class="alert alert-info">
                <i class="fas fa-info-circle"></i> フォロー中のユーザーのツイートがありません。ユーザーをフォローして最新情報を確認しましょう！
            </div>
        @endif
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadMoreTweets() {
        ...
        fetch(`/following-tweets/load-more?page=${nextPage}`)
